    ...
    it('should infer variantIds if they are not provided', done => {
      Component.fetchById(state, 'label')
        .then(data => {
          ...
          assert.equal(data.variantIds[0], 'label/label-handlebars.hbs')
          assert.equal(data.variantIds[1], 'label/label-html.html')
          assert.equal(data.variantIds[2], 'label/label-marko.marko')
          assert.equal(data.variantIds[3], 'label/label-pug.pug')
          assert.equal(data.variantIds[4], 'label/label-react.jsx')
          assert.equal(data.variantIds[5], 'label/label-vue-js.js')
          assert.equal(data.variantIds[6], 'label/label-vue-sfc.vhtml')
          done()
        })
        .catch(done)
    })
