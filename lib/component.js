...
async function findVariationIds (componentsPath, componentId, variationGlob = '*') {
  const variationsPath = ComponentUtil.componentIdToVariationsPath(componentsPath, componentId)
  const pathGlob = path.join(variationsPath, variationGlob)
  const variationPaths = await glob(pathGlob)
  const variationIdFromVariationFilePath = R.partial(ComponentUtil.variationIdFromVariationFilePath, [componentsPath, componentId])
